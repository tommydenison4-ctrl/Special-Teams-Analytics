...
export default {
  async fetch(request) {
    if (request.method === 'GET') {
      return json({ ok: true, service: 'roster-importer', version: '7.3', method: 'POST' });
    }
    ...
    try {
      ...
      if (!response.ok) {
        return json({ error: `The roster website returned HTTP ${response.status}.` }, 502);
      }

      const html = await response.text();
      if (html.length > 8_000_000) {
        return json({ error: 'The roster page was too large to import safely.' }, 413);
      }
      ...
      return json({ source: response.url || rosterUrl.href, count: players.length, players });
    } catch (error) {
      const message =
