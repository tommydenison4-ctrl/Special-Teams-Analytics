...
export default async function handler(req,res){
  ...
  try{
    ...
    if(r.status===404) return sendJson(res,404,{error:'No Southeastern Louisiana roster.json found in Opponents/SoutheasternLA.'});
    if(!r.ok){
      const detail=await r.text();
      return sendJson(res,r.status,{error:`Supabase Southeastern Louisiana roster read failed (${r.status}): ${detail||'unknown error'}`});
    }
    const data=proxyRosterImages(await r.json());
    return sendJson(res,200,data);
  }catch(error){
    return sendJson(res,500,{error:error?.message||'Southeastern Louisiana roster read failed.'});
  }
}
