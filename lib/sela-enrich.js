...
function highResImage(raw, pageUrl) {
  ...
  try {
    ...
    const allowed = host === OFFICIAL_HOST || host.endsWith('.lionsports.net') || host === 'images.sidearmdev.com' || host.endsWith('.sidearmdev.com') || host === 'images.sidearmsports.com' || host.endsWith('.sidearmsports.com') || host.endsWith('.cloudfront.net');
...
async function enrichOne(player) {
  ...
  try {
    ...
    console.error('SELA profile enrichment failed:', player.profile, error?.message || error);
    return {
      ...player,
      imageVerifiedAt: new Date().toISOString(),
      bioVerifiedAt: new Date().toISOString(),
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const internal = req.__internalSelaEnrich === true;
    if (!internal) {
