...
const PHOTO_REFRESH_VERSION = 'sela-bios-2026-09-12-v1';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const publicRosterRefresh = body.publicSelaRefresh === true || body.publicRosterRefresh === true;
    if (!publicRosterRefresh && String(body.adminKey || '') !== adminKey) {
    ...
    // Public refresh processes every official Southeastern Louisiana profile once per refresh version.
    // Admin enrichment keeps the existing photo/bio completion behavior.
    const pendingIndexes = [];
    for (let i = 0; i < players.length; i++) {
      const p = players[i];
      const needsPublicPhotoRefresh = publicRosterRefresh && p.profile && p.photoRefreshVersion !== PHOTO_REFRESH_VERSION;
      const needsAdminEnrichment = !publicRosterRefresh && p.profile && (!p.imageVerifiedAt || !p.image || !p.bio);
    ...
    if (!pendingIndexes.length) {
      ...
      return sendJson(res, 200, {
        ...
        completed: publicRosterRefresh ? players.filter(p => !p.profile || p.photoRefreshVersion === PHOTO_REFRESH_VERSION).length : players.filter(p => !p.profile || p.imageVerifiedAt).length,
        photos,
        bios
      });
    }
    ...
    const photos = players.filter(p => p.image).length;
    const bios = players.filter(p => p.bio).length;
    const completed = publicRosterRefresh ? players.filter(p => !p.profile || p.photoRefreshVersion === PHOTO_REFRESH_VERSION).length : players.filter(p => !p.profile || p.imageVerifiedAt).length;
    const remaining = publicRosterRefresh ? players.filter(p => p.profile && p.photoRefreshVersion !== PHOTO_REFRESH_VERSION).length : players.filter(p => p.profile && (!p.imageVerifiedAt || !p.image || !p.bio)).length;
